...
export const UpdateSessionByuserId = (userId) =>
  prisma.session.updateMany({ where: { userId }, data: { deviceInfo: null, revokedAt: new Date() } });

export const AddDeviceInfoByuserId = (userId, deviceInfo) =>
  prisma.session.updateMany({ where: { userId }, data: { deviceInfo: deviceInfo } });

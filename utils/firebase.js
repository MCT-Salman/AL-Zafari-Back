...
export const initializeFirebase = () => {
  try {
    ...
    if (!serviceAccount.project_id || !serviceAccount.private_key || !serviceAccount.client_email) {
      logger.warn('🔥 Firebase configuration incomplete. Push notifications will be disabled.');
      logger.warn('💡 To enable push notifications:');
      logger.warn('   1. Go to Firebase Console > Project Settings > Service Accounts');
      logger.warn('   2. Generate a new private key');
      logger.warn('   3. Add the credentials to your .env file');
    ...
    if (!serviceAccount.private_key.includes('BEGIN PRIVATE KEY')) {
      logger.warn('🔥 Firebase private key format is invalid. Push notifications will be disabled.');
      logger.warn('💡 Make sure the private key includes the full PEM format with headers and footers');
      return null;
    }
    ...
    logger.error('❌ Failed to initialize Firebase Admin SDK:', { message: error?.message, stack: error?.stack });
    return null;
  }
};
...
export const getMessaging = () => {
  try {
    ...
    logger.error('Failed to get Firebase Messaging instance:', { message: error?.message, stack: error?.stack });
    return null;
  }
};
  ...
  try {
    ...
    if (response.failureCount > 0) {
      response.responses.forEach((resp, idx) => {
        if (!resp.success) {
          logger.error(`Failed to send to token ${fcmTokens[idx]}:`, { error: resp.error });
        }
      });
    }
    ...
    logger.error('❌ Failed to send push notifications to multiple devices:', { message: error?.message, stack: error?.stack });
...
export const validateFCMTokenAdvanced = async (fcmToken) => {
  try {
    ...
    logger.error('Invalid FCM token:', { message: error?.message, stack: error?.stack });
    return false;
  }
};
